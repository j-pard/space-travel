...
class VenusScene extends Phaser.Scene {
    ...
    preload(){
        preloadGame(this);
        //load map
        this.load.image("tiles", './img/tilesetVenus.png');
        this.load.tilemapTiledJSON("map", './map/venus.json');
        this.load.image('bg1', './environment/bg-1.png');
        this.load.image('bg2', './environment/bg-9.png');
        this.load.image('bg3', './environment/bg-10.png');

        this.load.audio('backmusic', './audio/themevenus.mp3');
    }
    ...
    create(){
        ...
        bg3 = this.add.tileSprite(0 ,0, this.cameras.main.width / 4, this.cameras.main.height / 4, "bg3");
